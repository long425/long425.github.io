...
$(".point").css("background-color","#74A8ED")
			.css("color","#FFFFFF")
			.css("transform","scale(1.5)");

$("#lbt>img").each(function(i){
	$("#lbt>img").eq(i).css("top","i*368+px")
})
...
function moveImage(isAnimate) {
	...
	$("#lbt>img").each(function(i){
		if(currentIndex == 0 && !isAnimate){
			$("#lbt>img").eq(i).css("transition","none");
		}else{
			$("#lbt>img").eq(i).css("transition","top 0.5s ease-in-out");
		}
		$("#lbt>img").css("top","(i - currentIndex)*368 +px");
	})
